  ...
  useEffect(() => {
    ...
    Promise.all([fetchResources, fetchCategories])
      .then(([resourcesResponse, categoriesResponse]) => {
        let fetchedResources: Resource[] = resourcesResponse.data;
        const fetchedCategories: Category[] = categoriesResponse.data;

        setCategories(fetchedCategories);

        const resourcesWithTruncatedContent = fetchedResources.map(resource => ({
          ...resource,
          truncatedContext: truncateContent(resource.context, 50),
          truncatedTeachings: truncateContent(resource.teachings, 50),
        }));
        setResources(resourcesWithTruncatedContent);
      })
      .catch(error => {
...
<div className="search-blog ">
  ...
  <img className="search-svg" src="/search.svg" alt="Search" />
      ...
      {isLoggedIn && (
        <Link to="/create-resource" className="create-post-button">
          +
        </Link>
      )}
              ...
              onMouseLeave={(e) => {
                ...
                <div style={{ padding: '0' }}>
                  ...
                  <p style={{ 
                    ...
                    Teaches: {resource.truncatedTeachings}
                  </p>
